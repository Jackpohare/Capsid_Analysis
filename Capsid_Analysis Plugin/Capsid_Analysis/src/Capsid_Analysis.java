...
public class Capsid_Analysis  implements PlugIn,ActionListener {
	...
	protected static final int CB_SCATTER_POS=CB_EMPTY+1,CB_INTENSITY_POS=CB_SCATTER_POS+1,CB_FREQUENCY_POS=CB_SCATTER_POS+2,
	            CB_SCATTER_ALL = CB_SCATTER_POS + 3, CB_INTENSITY_ALL = CB_SCATTER_POS + 4, CB_FREQUENCY_ALL = CB_SCATTER_POS + 5;
	...
	String sVersion =" (v1.2.08, 10-Dec-2018)";
...
public ResultsTable rt;;
	...
	public void actionPerformed(ActionEvent e){
		...
		case "Find Maxima and generate ROI":
            ...
            if (ThresholdMode.THRESHOLD_MEAN== settings.thresholdMethod){
            	...
            	this.settings.redThreshold = (settings.redBackgroundMean*3.1415926*((settings.pointDiameter/2.0)*(settings.pointDiameter/2.0))) *settings.backgroundDevFactor;
            	this.settings.greenThreshold = (settings.greenBackgroundMean*3.1415926*((settings.pointDiameter/2.0)*(settings.pointDiameter/2.0))) *settings.backgroundDevFactor;
            }
	...
	public  JFreeChart createClusteredChart(String title, String categoryAxisLabel, String valueAxisLabel, IntervalXYDataset dataset) {
	...
	public JFreeChart createMyAreaChart(final DefaultXYDataset dataset) {
		        ...
		        final JFreeChart chart = ChartFactory.createXYAreaChart(
		            "Particle Distribution Comparison",             // chart title
...
//		        subtitle.setSpacer(new Spacer(Spacer.RELATIVE, 0.05, 0.05, 0.05, 0.05));
	...
	private void DoFrequencyPlot(AnalysisSettings s, double bucketWidth, boolean bPositivesOnly, boolean bRedOnly, boolean bGreenOnly,
			String sTitle) {
	    // Get only positivies
	    int i;
	    double[] red;
	    ...
	    if (s.debug > 0) { IJ.log("DoPositiveFrequencyPlot - max positive green: " + maxGreen); }
	    ...
	    if (s.debug > 0) { IJ.log("DoPositiveFrequencyPlot - top bin: " + nTopBin); }


	    // NUmber of bins
	    int nBinCount = (int) Math.floor((nTopBin / bucketWidth) + 1);
	    if (s.debug > 0) { IJ.log("DoPositiveFrequencyPlot -  bin count: " + nBinCount); }
	    ...
	    // Do green counts as individual lines with a think line to simulate a column chart
	    if (!bRedOnly) {
	        p.setColor(new Color(0,255,0,128));
	        p.setLineWidth(10);
	        for (i = 0; i < Bins.length; i++) {
	            double[] cx = {Bins[i]+(bGreenOnly?0:-1) ,Bins[i]+(bGreenOnly?0:-1) }, cy = {0,greenBinCounts[i]};
	            p.setLineWidth(10);
	            p.setColor(new Color(0,255,0,128));
	            if (cy[1] > 0) {
	                p.setLineWidth(10);
	    ...
	    p.setLineWidth(10);
	    for (i = 0; i < Bins.length; i++) {
            double[] cx = {Bins[i]+(bRedOnly?0:1) ,Bins[i] +(bRedOnly?0:1) }, cy = {0,redBinCounts[i]};
	        if (cy[1] > 0) {
	            p.setLineWidth(10);
	            p.setColor(new Color(255,0,0,128));
	            p.add("line", cx, cy);
	    ...
	    p.setLineWidth(1);
	    p.add("circle", Bins, greenBinCounts);

	    p.setColor(Color.red, Color.red);
	    p.setLineWidth(1);
	    p.add("circle", Bins, redBinCounts);
	    */
	    ...
	    if (!bRedOnly){
		    p.setColor(Color.green, Color.green);
		    p.setLineWidth(1);
		    if(settings.debug>0){ IJ.log("Fitting green...");}
	    ...
	    if (!bGreenOnly) {
	        p.setColor(Color.red, Color.red);
	        p.setLineWidth(1);
		    if(settings.debug>0){ IJ.log("Fitting red...");}

	        DoPlotFit(Bins, redBinCounts, p, s,   bPositivesOnly?Color.red:Color.black,  (bPositivesOnly?0.9:0.05) );
	    }
...
//	    	newChart = new  DistributionChart("MULTI", particles, s, ChartType.MULTI);
	...
	public void DoPlotFit(double[] x,double[] y,Plot p,AnalysisSettings s,Color paramColor, double xParam) {
	    ...
	    double[] params = cf.getParams();
	    for (int i = 0; i < params.length; i++) {
	        if (s.debug > 0) { IJ.log("param " + i + ": " + params[i]); }
	    }
...
/*
    if (cbx.get(CB_SCATTER_POS).getState()) {
        DoScatterPlot(settings, true);
    }
    if (cbx.get(CB_SCATTER_ALL).getState()) {
        DoScatterPlot(settings, false);
    }
    ...
    if (cbx.get(CB_INTENSITY_ALL).getState()) {
        DoSortedIntensityPlot(settings,false);
    }
    // settings.bucketWidth = 0;
    if (cbx.get(CB_FREQUENCY_POS).getState()) {
        DoFrequencyPlot(settings, settings.bucketWidth,true,false,false,"Frequency Plot: Positives" );
    }
    if (cbx.get(CB_FREQUENCY_ALL).getState()) {
    	if (!settings.bGreyscale) {
    	    DoFrequencyPlot(settings, settings.bucketWidth, false,true,false ,"Frequency Plot: All Red Particles");
    	}
    	DoFrequencyPlot(settings, settings.bucketWidth, false, false, true, "Frequency Plot: All Green Particles");
    }
    */
	...
	/**
	 * Handles when user clicks canvas on point that is not in ROI
	 * Pops up menu that offers adding ROI manually 
	...
	public void DoResultsTableSort() {
		...
		String[] sortOptions ={"Green Mean","Red Mean","ID","Raw Green Intensity","Raw Red Intensity","Total Raw","x","y"};
	    ...
	    if (SortOrder == "Raw Red Intensity") { nSortCol = 4; }
	    if (SortOrder == "ID") { nSortCol = 0; }
	    if (SortOrder == "x") { nSortCol = 2; }
	    if (SortOrder == "y") { nSortCol = 3; }
	    if (SortOrder == "Green Mean") { nSortCol = 11; }
	    if (SortOrder == "Red Mean") { nSortCol = 9; }
	    if (SortOrder == "Total Raw") { nSortCol = 8; }
		...
		private void DoScatterPlot(AnalysisSettings s, boolean bPositivesOnly) {
		  if (s.debug > 0) { IJ.log("Doing scatter plot"); }
		  
		    int i, n, j, k,nPos = 0,nNeg = 0;	// Number of points to be plotted
		    // Calculate number of points to plot - i.e. excluding those with one or other color with score of 0
		    for (i = 0; i < particles.size(); i++) {
		        if (particles.get(i).IsRedPositive() ) { nPos++; }
		        else { nNeg++; }
		    }
...
*/
	...
	public  void DoShowROI(AnalysisSettings s) {
	   ...
	   // boolean bNoisey = cbx.get(CB_NOISEY).getState();
	    ...
	    if (s.debug > 0) { IJ.log("Showing " + (bRed ? "red, " : "") + (bGreen ? "green, " : "") + (bBoth ? "Both, " : "") +(bOverlaps?"overlapped, ":"")); }
	    ...
	    for (int i = 0; i < particles.size(); i++) {
	        ...
	        /*
	        if (bNoisey) {
	            if ((p.rawred >s.redBackground && p.rawred <= s.redThreshold && p.rawgreen <= s.greenThreshold) || 
	                (p.rawgreen > s.greenBackground && p.rawgreen <= s.greenThreshold && p.rawred <= s.redThreshold)) {
	                // IJ.log("Adding " + s.id[i]);
	                s.overlay.add(p.roi, String.valueOf(p.id));
	            }
   ...
   // s.resultsSummary.setHeading(0, "");
	...
	public void DoUpdateResults(){
        ...
        if (ThresholdMode.THRESHOLD_MEAN== settings.thresholdMethod){
        	...
        	this.settings.redThreshold = (settings.redBackgroundMean*3.1415926*((settings.pointDiameter/2.0)*(settings.pointDiameter/2.0)));
        	this.settings.greenThreshold = (settings.greenBackgroundMean*3.1415926*((settings.pointDiameter/2.0)*(settings.pointDiameter/2.0)));
        }
	...
	public double[][] DrawDistribution(double []x, double []y,Plot p, Color color, AnalysisSettings s){
	    ...
	    for (int i = 0; i < fitX.length; i++) {
			...
			if ( s.debug > 2 ) { IJ.log(i+", "+fitX[i]+", "+fitY[i]); }
	...
	public void drawParams(Plot p, double[] params, Color paramColor, double xParam) {
	    ...
	    //p.drawNormalizedLine(x - xspace, 1-y - yspace, x + 6 * xspace, 1- y - yspace);
	...
	/**
	 * Given an image x,y co-ordinate, return the Roi if co-ordinates is inside one, else return null
	 * @param x Image x
	 * @param y Image y
	...
	public ParticleList GetMaxima(){
	    ...
	    if (settings.debug > 0) { IJ.log("Maxima count=" + maxima.xpoints.length); }
		...
		for(int i=0;i<particles.size();i++){
			...
			for(int k=0;k<points.length;k++){
				if(points[k].y*width+points[k].x >= hits.length ) {
					...
					try {
				if (!hits[points[k].y*width+points[k].x]) { backgroundCount--; }
	...
	/**
	 * Generates frequency histogram column plot for given red and green counts using SD from mean as X axis
	 * 
	 * @param xGreen
	 * @param yGreen
	 * @param xRed
	...
	public void OverlappedFrequencyChart(double []green, double [] red, AnalysisSettings s){
		...
		if (s.debug > 2){ IJ.log("values=");
			for(int i=0;i<green.length;i++){
				IJ.log(i+", "+green[i]+","+red[i]);
			}
		}
		
		// Get the bucket width then we are going to sum based on centering on the mean - i.e. the bucket around the mean
        ...
        if (s.debug > 0){ IJ.log("Offset = "+offset); }
...
//        	if (s.debug > 2){ IJ.log("" + i + ", " + bucket[i] +", " + xStdDev[i]); }
        ...
        GetFrequencyDistirubtion( bucket, green, greenBinCounts,bucketWidth);
        GetFrequencyDistirubtion( redBucket, red, redBinCounts,bucketWidth);
    	    
       if (s.debug > 2){ IJ.log("\nBuckets"); }
       for(int i=0;i<nBucketCount;i++){
        	if (s.debug > 2){ IJ.log("" + i + "," + xStdDev[i]+ ","+bucket[i]+","+greenBinCounts[i]+","+redBucket[i]+","+redBinCounts[i]); }
        }
	...
	/**
	 * SetResults does all the hard work for showing results, counting particles, get particle intensities etc..
	 * Clears and re-populates the results table
	 * @param s Current settings
	 */
	public void SetResults(AnalysisSettings s) {
	    Roi roi; Color roiColor;
	    if (settings.debug > 0) { IJ.log("Getting results - " + particles.size() + " particles"); }
	    
	    if (settings.debug > 0) { IJ.log("Background red: " + s.redBackground + ", green: " + s.greenBackground); }
	       ...
	       rt.addValue("Status", p.GetStatus());
	        ...
	        if (p.IsRedPositive()) {
	            ...
	            if ( p.IsGreenPositive()) {
	                if (s.bFillROI) {
	                    roi.setFillColor(new Color(255, 140, 0));
	                } else {
	                    roi.setStrokeColor(new Color(255, 140, 0));
	                }
	            } else {
	            //  Red with raw green not above greenbackground so this is red only
	                if (s.bFillROI) {
	                    roi.setFillColor(new Color(255, 0, 0));
	                } else {
	        ...
	        if (p.IsGreenPositive()) {
	            ...
	            if ( !p.IsRedPositive()) {
	                ...
	                } else { roi.setStrokeColor(new Color(0, 255, 0)); }
	            }

	        } else  {
	            rt.addValue("HasGreen", "No");
	        }
*/
	 ...
	 public void SetRoi(List<Particle> pList){
		...
		for(int i=0;i<pList.size();i++){
			...
			if(p==null){
				...
				p.roi.setName(String.valueOf(p.id));
				if ( settings.debug > 3 ) {IJ.log("ROI "+i+": "+p.roi.toString()); }
				rm.addRoi(p.roi);
			}
		}
	}
	...
	public void SetSummary() {
		 AnalysisSettings s = this.settings;
	        ...
	        String[] Labels = {"Image Name", "Total Number of maxima found", "Noise Tolerance", "ROI Diameter", "ROI Red Average Background",
	        		      "ROI Green Average Background",
	                      "# Red ROI",
	                      "# Green ROI",
	                      "# ROI both Red and Green",
	                      // "# Noisey Red (between background red and  red threshold) ","# Noisey Green",
	                      "# Empty ROI",
	                      "Max Red ROI Intensity", "Max Green ROI Intensity",
	                      "Background Red Mean",
	                      "Background Red StdDev",
	                      "Background Green Mean",
	                      "Background Green StdDev",
	                      "Threshold Method",
	                      "Red Threshold",
	                      "Green Threshold"
	                      };
	        ...
	        String[] Values = {s.image.getTitle(), String.valueOf(particles.size()),String.valueOf(s.noiseTolerance), 
	        		String.valueOf(s.pointDiameter),String.valueOf(s.redBackground), String.valueOf(s.greenBackground),
	                      ...
	                      String.format("%.2f", settings.greenBackgroundStdDev),
	                      ""+settings.thresholdMethod,
	                      String.format("%.2f", settings.redThreshold ),
	                      String.format("%.2f", settings.greenThreshold)
	                        };
	...
	public void SetThresholdFromParticle(Particle p,String channel,boolean setBelow){
		if (settings.thresholdMethod==ThresholdMode.THRESHOLD_MEAN){
			if (channel=="green") {
				...
				if (setBelow){settings.redThreshold = p.redmean*1.005;}
			}
		} else {
			if (channel=="green") {
				settings.greenThreshold = p.rawgreen;
				if (setBelow){settings.greenThreshold = p.rawgreen*1.005;}
			} else {
				settings.redThreshold = p.rawred;
				if (setBelow){settings.redThreshold = p.rawred*1.005;}
			}
	...
	private void Setup(){
	    ...
	    settings.redBackground = stats.mean * (settings.pointDiameter / 2.0) * (settings.pointDiameter / 2.0) * 3.1415926;
	    ...
	    // repeat for green
	    if ( settings.bGreyscale ) { 
	    	   if (settings.debug > 0) { IJ.log("Setting greyscale weightings"); }
	    ...
	    settings.greenBackground = stats.mean * (settings.pointDiameter / 2.0) * (settings.pointDiameter / 2.0) * 3.1415926;
	...
	public void ShowAllChannels(){
		...
		if(stack!=null){IJ.log("Stack size: "+settings.image.getImageStackSize());}
	...
	String [] plotList = new String[] {"All Particles Scatter Plot","All Particles Sorted Intensity Plot",
										"All Particles Green Distribution Plot"
										,"All Particles Red Distribution Plot",
	...
	private void ShowDialog(AnalysisSettings s){
	    ...
	    cp.addChoice("Threshold method:", new String[] {"Use area based threshold compared to ROI area","Compare background mean to ROI mean"}, "Compare background mean to ROI mean");
	    ...
	    /*
	    bt = new Button("Show ROI");
	    bt.addActionListener(this);
	    pnl = new Panel();
	    pnl.add(bt);
	    
	    bt = new Button("Hide ROI");
	    bt.addActionListener(this);
	    pnl.add(bt);
	    */
	    /*
	    bt = new Button("Red Channel");
	    bt.addActionListener(this);
	    pnl.add(bt);
	    
	    bt = new Button("Green Channel");
	    bt.addActionListener(this);
	    pnl.add(bt);
*/
...
*/
	    ...
	    cp.addCheckboxGroup(1, 2, new String[] {"Show ROI", "Fill ROI"},
                new boolean[]{ true, true});
	    
	    cp.addCheckboxGroup(1, 4, new String[] {"Red Only (____)", "Green Only (____)", "Both (_____)", "Empty (____)"},
	    ...
	    pnl = new Panel();	    bt = new Button("Show Selected Plot");
	    bt.addActionListener(this);
	    pnl.add(bt);
	    cp.addPanel(pnl);

	    /*cp.addCheckboxGroup(2, 3, new String [] {"Scatter Plot (positives)", "Intensity Plot (positives)", "Frequency Plot (positives)",
	                                    "Scatter Plot (all)", "Intensity Plot (all)", "Frequency Plot (all)"},
	                        new boolean [] {true, true, true,false,false,false});
*/
	//	cp.addSlider("Frequency Plot Bucket width (0=auto):", 0.0, 5000.0, s.bucketWidth);
	...
	public void ShowInfo(Particle p) {
		 ...
		 ResultsTable rt = new ResultsTable();
	        ...
	        String[] Labels = {"ID", "Status", "x","y",
	        		      "Red Intensity", "Red Mean", "Red StdDev", "% of max red",
	        ...
	        String[] Values = {""+p.id,p.GetStatus(),String.valueOf(p.x),String.valueOf(p.y),
	        		...
	        		String.format("%.1f",p.redPct),
	        		""+p.redRank,""+p.redBelow,
	        		String.format("%.0f",p.rawgreen),String.format("%.1f",p.greenmean),String.format("%.1f",p.greenstdDev),
	        		String.format("%.1f",p.greenPct),
	        		""+p.greenRank,""+p.greenBelow,
	                        };
	        String[] background = { "","","","",
	        		...
	        		"","","",
	        		String.format("%.0f",settings.greenBackground),String.format("%.1f",settings.greenBackgroundMean),String.format("%.1f",settings.greenBackgroundStdDev),
	        		"","",""
	  	        };
	 ...
	 public void sorting(int nSortingColumn, boolean bAscending, ResultsTable rt) 
	{
			...
			if (settings.debug > 0) { IJ.log("Sorting Results Table: Preparation..."); }
			for (i=0; i<colNumber;i++)
			{
			   //  IJ.log("Preparing column " + i);
			     for (j = 0; j < len; j++) {
		//		    IJ.log("Getting row " + j);
				        cellValue = (i==1 )?rt.getStringValue(i, j):
				        	String.format("%.4f",rt.getValueAsDouble(i, j));
	               if(j==0){
			...
			if (settings.debug > 0 ) { IJ.log("Sorting Results Table: Sorting..."); }
			...
			if (settings.debug > 0) { IJ.log("Sorting Results Table: Updating Table..."); }
			for (i = 0; i < colNumber; i++) {
			...
			if (settings.debug > 0) { IJ.log("Updating results"); }
			rt.show("Results");
			...
			if (rtp != null) {
			    rtp.scrollToTop();
			} else { IJ.log("Could not get tesults text panel");}
		}
	...
	public void UpdateControlPanel() {
	    ...
	    /*
	    // Work out what percentage the threshold is for max of each of red and green and set
	    doulbe redPct = s.redThreshold * 100.0 / s.maxRed;
	    var greenPct = s.greenThreshold * 100.0 / s.maxGreen;
	    var sFields = gd.getStringFields();
	    if (s.debug) {
	        IJ.log("UpdateControlPanel: " + sFields.length + " string fields");
	    }
