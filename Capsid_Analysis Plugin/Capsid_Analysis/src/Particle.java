...
public class Particle { 
	static AnalysisSettings settings;
		...
		public Particle(int id, double x,double y, AnalysisSettings s){
			...
			this.x = x;
			this.y = y;
	        this.roi = new OvalRoi(x - settings.pointDiameter / 2.0, y - settings.pointDiameter / 2.0, settings.pointDiameter * 1.0, settings.pointDiameter * 1.0);
		        ...
		        for (int x = 0; x < r.width; x++) {
		            if (mask.getPixel(x, y) != 0) {
		                ...
		                if ( settings.bGreyscale ) {
		                	red = 0; blue = 0; green=px;
		                }
		                // if (i == 0) { IJ.log((r.x + x) + "  \t" + (r.y + y) + "  \t" + px.toString(16) + "  \t" + red + "  \t" + green + "  \t" + blue); }
		               rgb[0] += red;
		               rgb[1] += green;
		               rgb[2] += blue;
		               nPixelCount += 1;
		...
		/**
		 * @return TRUE if particle is neither green nor red positive else FALSE (particle is red or green or both)
		 */
		...
		/**
		 * @return Returns either the raw red intensity of this particle or it's mean, depending on the current thresholding method
		 */
		...
		/**
		 * Sets all the key measures for this ROI:
		 * raw red & green, mean & stdDev red & green
		 * Also sets pixels
		 */
		...
		public void writePixels(Writer writer, String channel) throws IOException{
			...
			for(int i=0;i<pixels.length;i++){
				int px = pixels[i];
				if (channel=="red") {  px = (px & 0xff0000)>>16; }
				if(channel == "green") { px= (px & 0xff00) >> 8 ;}
