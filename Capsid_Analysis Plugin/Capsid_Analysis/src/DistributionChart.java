...
public class DistributionChart {
	...
	private double maxYAxis = -1;

	JFreeChart maxChart = null;

	public DistributionChart(String title, ParticleList particles, AnalysisSettings settings, ChartType type) {
		if (IJ.debugMode ) {
		...
		_title = title;
		// Get the values of only positivies (i.e. equal number of red and green as positives have to
		// be both positive green and positive red)
		...
		if (IJ.debugMode ) {
			IJ.log("Chart counts - red: "+red.length+", green: " + green.length);
			IJ.log(" Green bucketWidth = " + GreenBucketWidth);
			IJ.log(" Red bucketWidth = " + RedBucketWidth);
		}
		...
		if (IJ.debugMode) {
			IJ.log("DistributionChart - top bin: " + nTopBin);
	...
	private void addMultilineTextAnnotations(XYPlot plot, double x, double y, String label, Color color) {
		...
		for (int i = 0; i < lines.length; i++) {
			XYTextAnnotation annotationLabel = new XYTextAnnotation(lines[i], x,
					y - (i + 1) * XYTextAnnotation.DEFAULT_FONT.getSize());
	...
	protected void BuildMultiChart() {
		...
		if (settings.debug > 0) {
			IJ.log("Max positive green: " + maxGreen + "\nMax positive red: " + maxRed);
		}

		// create subplot 1...
		if (settings.debug > 0) {
			IJ.log("Create green plot");
		}
...
//	        return new JFreeChart("CombinedDomainXYPlot Demo",    JFreeChart.DEFAULT_TITLE_FONT, plot, true);
	...
	/**
	 * Create a panel containing a frequency historgram using the given data
	 * @param color
	 * @param x
	 * @param y
	 * @param values
	...
	private JPanel createPanel(Color color, double[] x, double[] y, double[] values, String key, double maxValue,
		...
		if (IJ.debugMode) {
			IJ.log("Setting axis from " + lowerBound + " to " + (xAxis.getUpperBound() + lowerBound));
		}
		// SHift lowerbound - note that we also shift upperbound to ensure scale stays
		// the same
	...
	public JPanel createPanelFrequencyPlot(String key, double[] Bins, double[] Counts, double threshhold, Color color,
			double[] CountsBelowThreshold, XYPlot plot) {
		if (settings.debug > 0) {
		...
		XYToolTipGenerator xyToolTipGenerator = new XYToolTipGenerator() {
			public String generateToolTip(XYDataset dataset, int series, int item) {
				...
				stringBuilder.append(String.format("Mean Range:%.1f - %.1f<br/>", x1.intValue() - BucketWidth / 2,
						x1.intValue() + BucketWidth / 2));
	...
	private JPanel createPanelSortedIntensityPlot(boolean bAll) {
		...
		// We now have x (red) and y (green) in red sorted order with index as array of
		// indices
	...
	public void DoublePlot() {
		...
		// Need to work out x axis offset so that means align - we offset either red or
		// green to ensure X-axis remains positive
	...
	private void DrawChart(XYPlot plot) {
		...
		this.chart.draw(image.createGraphics(), new Rectangle2D.Float(0, 0, imp.getWidth(), imp.getHeight()));
		imp.setImage(image);
		imp.show();
	...
	public void FrequencyPlot(String title, double[] values, double threshhold, Color color,
			double[] BelowThresholdValues) {
		if (settings.debug > 0) {
			IJ.log("\nFrequencyPlot");
		}
		...
		JPanel chartPanel = createPanelFrequencyPlot(title, Bins, Counts, threshhold, color, CountsBelowThreshold,
				plot);
		...
		// thresholdSlider.addChangeListener(new
		// HandleThresholdSlider(this,plot,color));
	...
	protected void getBinCounts() {
		...
		if (settings.debug > 0) {
			IJ.log("Max Green frequency = " + maxGreenFrequency + ",  max Red Frequency = " + maxRedFrequency);
		}
	...
	public double[] getBins(double[] values) {
		...
		// Work out bins based on Max value - bins go from 0 in binInterval steps to
		// first binInterval above greenMax (.e.g 15 is max & interval is ten then it is
		// 0,10,20)
		// get top limit
		// but if top bin would go above 255 then we ensure bins are centred such that
	...
	public void SingleChart() {
		...
		BasicMultiLineXYTextAnnotation newLabel = new BasicMultiLineXYTextAnnotation(
				"Red Mean = " + utils.Mean(red) + "\nStdDev = " + utils.StdDev(red, utils.Mean(red)), utils.Mean(red),
				30);
	...
	public void SortedIntensityPlot(boolean bAll) {
		...
		JFrame frame = new JFrame(
				"Sorted Intensity Plot (" + (bAll ? "All" : "Positive") + " ROI): " + settings.image.getTitle());
		frame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		...
		if (bAll) {
			...
			// Get only positivies (i.e. equal number of red and green as positives have to
			// be both positive green and positive red)
			red = _particles.GetPosRed();
			green = _particles.GetPosGreen();
			settings.GetMax(_particles);
